...
async def send_morning_notif(dp: Dispatcher):
    ...
    for user in await db.get_users_notif():
        ...
        if len(tasks) != 0:
            m = '👋 Доброе утро. Новый день - новые возможности!' \
            ...
            m = '👋 Доброе утро. Новый день - новые возможности! Вы ещё не ' \
...
async def send_evening_notif(dp: Dispatcher):
    for user in await db.get_users_notif():
        m = '👋 День подходит к концу. Не забудьте выставить цели на завтра! ' \
