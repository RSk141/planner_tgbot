...
async def support(message: types.Message):
    await message.answer(_('Вы хотите обратится в поддержку?'), reply_markup=await confirm_support())


async def call_support(call: types.CallbackQuery):
    await call.message.edit_reply_markup()
    await call.message.answer(_('👋 Напишите свои предложения, вопросы или проблему, с которой столкнулись\n'
...
async def send_to_admin(message: types.Message, state: FSMContext):
    ...
    await message.answer(_('Ваше сообщение успешно доставлено. Ожидайте ответ от администратора!'))
    await state.finish()
...
async def reply_to_user(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(_('Введите ответ пользователю'))
...
async def send_reply(message: types.Message, state: FSMContext):
    ...
    m = [
        _('⚠️ У вас новое сообщение от администратора ⚠️\n'),
        f'{reply_msg}'
    ]
